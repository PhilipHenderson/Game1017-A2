...
bool Engine::Init(const char* title, int xpos, int ypos, int width, int height, int flags)
{
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0)
	{
		...
		if (m_pWindow != nullptr) // Window init success.
		{
			...
			if (m_pRenderer != nullptr) // Renderer init success.
			{
				...
				SOMA::Init();
				TEMA::Init();
				/* This sets an aspect ratio that will be maintained, and actually resize all the sprites.
				* but make sure you set the window to resizable as I have done in the run */
	...
	SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1"); // Call this before any textures are created.
	//TEMA::RegisterTexture("Img/quit.png", "quit");
...
int Engine::Run()
{
	if (m_running) // What does this do and what can it prevent?
		return -1; 
	if (Init("GAME1017_Assignment2", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, SDL_WINDOW_RESIZABLE) == false)
		return 1;
